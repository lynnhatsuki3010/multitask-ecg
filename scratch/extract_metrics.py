...
print(f"{'Experiment':<10} | {'Arrhy F1 (Tuned)':<16} | {'MI F1 (Tuned)':<13} | {'IMI AUPRC':<9} | {'Folder'}")
print("-" * 80)

for name, folder in runs:
    path = os.path.join("checkpoints", folder, "history.json")
    ...
    try:
        with open(path, "r") as f:
            h = json.load(f)
            # Find best epoch based on IMI AUPRC (since that was the monitor metric)
            best_epoch = max(h["val"], key=lambda x: x.get("auprc/mi/IMI", 0))
            
            # Print specific metrics: 
            # Arrhy Macro F1 (Tuned), MI Macro F1 (Tuned), IMI AUPRC
            arrhy_f1 = best_epoch.get("tuned/f1/arrhy/macro", best_epoch.get("f1/arrhy/macro", 0))
            mi_f1 = best_epoch.get("tuned/f1/mi/macro", best_epoch.get("f1/mi/macro", 0))
            imi_auprc = best_epoch.get("auprc/mi/IMI", 0)
            
            print(f"{name:<10} | {arrhy_f1:.4f}           | {mi_f1:.4f}        | {imi_auprc:.4f}    | {folder}")
