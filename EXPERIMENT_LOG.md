...
| LOSS-E04 | `Asymmetric Loss (ASL)`| 0.945 | 0.496 | 0.469 | 0.785 | `run_20260509_110546_hybrid-tf` | |

*(Reference Baseline NORM-E01: IMI AUPRC 0.525, Arrhy F1 0.830)*

**Stage 3 winner**: **LOSS-E03** (`Focal + GradIso`)

**Analysis (The "15-Epoch Illusion" & ASL Performance):**
...
3. **Phase 2 SOTA — Asymmetric Loss (ASL)**: `LOSS-E04` applied ASL (ICCV 2021) to all tasks. While ASL slightly improved IMI AUPRC (0.496 vs 0.493), it caused a significant drop in Arrhythmia F1 (0.785). ASL clips easy negative gradients, which is excellent for rare labels but disrupts the learning dynamics for majority/dominant classes (like NORM in Arrhythmia). Thử nghiệm chứng minh Focal + GradIso vẫn là kiến trúc cân bằng tốt nhất cho multi-task.
